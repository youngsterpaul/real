...
  name: string;
  price: number;
  numberOfPeople: number; // Added for type safety in calculation
}
interface Hotel {
  id: string;
  name: string;
...
const StarRatingDisplay = ({ rating, count, iconSize = 5 }: StarRatingDisplayProps) => {
  ...
  // const fractionalPart = rating - fullStars; // For complex partial star
    ...
    <div className="flex items-center gap-1">
      {[...Array(5)].map((_, i) => (
        <Star
          ...
          // Simple fill: fully filled if index is less than fullStars
...
          {/* --- Image Carousel Section & Main Content (Left Column on large screens) --- */}
          {/* order-1 on mobile, order-1 on desktop (to be followed by order-2 right column) */}
...
            </div>
           
            {/* --- MOBILE ONLY: Description Section (Mobile Order: 6) --- */}
            {/* Show only on small screens, desktop handles it in the main content flow */}
            {hotel.description && 
              <div className="bg-card border rounded-lg p-4 sm:p-3 mt-4 lg:hidden">
...
            }
            
             {/* --- Amenities Section (RED) (Mobile Order: 7) --- */}
...
            {/* --- Facilities (Room Types) Section (TEAL) (Mobile Order: 8) --- */}
...
            {/* --- Activities Section (ORANGE) (Mobile Order: 9) --- */}
...
            )}
            
            {/* --- Review Section (Full Rating/Submission) (Mobile Order: 10, Below Activities) --- */}
...
          {/* --- Detail/Booking Section (Right Column on large screens, Stacked on small) --- */}
          {/* order-2 on mobile (stacked below images), order-2 on desktop (right column) */}
...
            {/* Operating Hours/Availability Card (Teal border) (Mobile Order: 2) */}
...
            </div>
            
            {/* Contact Information Section (Mobile Order: 11, after all main content) */}
