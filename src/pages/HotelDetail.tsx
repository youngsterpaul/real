...
const HotelDetail = () => {
  ...
  const distance = position && hotel?.latitude && hotel?.longitude ? calculateDistance(position.latitude, position.longitude, hotel.latitude, hotel.longitude) : undefined;
  useEffect(() => {
    fetchHotel();
    ...
    try {
      ...
      if (error && id.length === 8) {
        const {
          data: prefixData,
          error: prefixError
        } = await supabase.from("hotels").select("*").ilike("id", `${id}%`).single();
        if (!prefixError) {
          data = prefixData;
          error = null;
        }
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      toast({
        title: "Link Copied!",
        description: user ? "Share this link to earn commission on bookings!" : "Share this hotel with others!"
      });
    } catch (error) {
      toast({
        title: "Copy Failed",
  ...
  return <div className="min-h-screen bg-background pb-20 md:pb-0">
      ...
      <div className="relative w-full overflow-hidden md:max-w-6xl md:mx-auto">
        ...
        <Button variant="ghost" onClick={() => navigate(-1)} className="absolute top-4 left-4 z-30 h-10 w-10 p-0 rounded-full text-white md:left-8" // Increased Z-index and adjusted md:left
      style={{
      ...
      size="icon">
        ...
        <Button variant="ghost" size="icon" onClick={handleSave} className={`absolute top-4 right-4 z-30 h-10 w-10 p-0 rounded-full text-white md:right-8 ${isSaved ? "bg-red-500 hover:bg-red-600" : ""}`} style={{
        backgroundColor: isSaved ? RED_COLOR : 'rgba(0, 0, 0, 0.5)'
      }} // Dark RGBA or RED if saved
      >
        ...
        // Teal bottom border for small/big screen
        marginTop: 0,
        width: '100%',
        ...
        if (api) api.on("select", () => setCurrent(api.selectedScrollSnap()));
          ...
          <CarouselContent>
            {displayImages.map((img, idx) => <CarouselItem key={idx}>
                <img src={img} alt={`${hotel.name} ${idx + 1}`} loading="lazy" decoding="async" className="w-full h-[60vh] md:h-96 lg:h-[500px] object-cover" // Ensure height consistency
            />
              </CarouselItem>)}
          </CarouselContent>
        </Carousel>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 md:p-8 z-20 text-white bg-gradient-to-t from-black/80 via-black/50 to-transparent px-[8px] py-[26px]">
          <h1 className="text-3xl sm:text-2xl font-bold mb-0">{hotel.name}</h1>
          {hotel.local_name}
        </div>
        
        {/* Dot indicators */}
        {displayImages.length > 1 && <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2 z-30">
            {displayImages.map((_, idx) => <div key={idx} className={`w-2 h-2 rounded-full transition-all ${current === idx ? 'bg-white w-4' : 'bg-white/50'}`} />)}
          </div>}
      </div>
          ...
          <div className="space-y-4 sm:space-y-3 order-1 lg:order-3">
            ...
            <div className="space-y-3 p-4 sm:p-3 border bg-card rounded-lg">
              ...
              <div className="flex items-start gap-2">
                <Clock className="h-5 w-5 mt-1" style={{
                color: TEAL_COLOR
              }} />
                <div>
                  <p className="text-sm sm:text-xs text-muted-foreground">Working Hours & Days</p>
                  <p className="font-semibold sm:text-sm">
                    {hotel.opening_hours || hotel.closing_hours ? `${hotel.opening_hours || 'N/A'} - ${hotel.closing_hours || 'N/A'}` : 'Not specified'}
                  </p>
                  <p className="text-sm text-muted-foreground mt-1">
                    <span className="font-medium">Working Days:</span>{' '}
                    {hotel.days_opened && hotel.days_opened.length > 0 ? hotel.days_opened.join(', ') : 'Not specified'}
                  </p>
                </div>
              </div>
              ...
              <div className="border-t pt-3 sm:pt-2">
                ...
                <p className="text-2xl sm:text-xl font-bold" style={{
                color: TEAL_COLOR
              }}>
                  {hotel.available_rooms !== null && hotel.available_rooms !== undefined ? `${hotel.available_rooms} Rooms` : 'Check Availability'}
                </p>
              ...
              {/* Book Now Button */}
              <Button size="lg" className="w-full text-white h-10 sm:h-9" onClick={() => {
              setIsCompleted(false);
              setBookingOpen(true);
            }} style={{
              backgroundColor: TEAL_COLOR
            }} onMouseEnter={e => e.currentTarget.style.backgroundColor = '#005555'} onMouseLeave={e => e.currentTarget.style.backgroundColor = TEAL_COLOR}>
                Book Now
              </Button>
            </div>
            
            {/* Contact Information Section (Always in the right column / bottom of mobile section) */}
            {(hotel.phone_numbers || hotel.email) && <div className="p-4 sm:p-3 border bg-card rounded-lg hidden lg:block"> {/* Hidden on small screen, visible on large */}
                <h2 className="text-xl sm:text-lg font-semibold mb-4 sm:mb-3">Contact Information</h2>
                <div className="grid grid-cols-1 sm:grid-cols-1 gap-2">
                  {hotel.phone_numbers?.map((phone, idx) => <a key={idx} href={`tel:${phone}`} className="flex items-center gap-2 px-4 py-3 border rounded-lg hover:bg-muted transition-colors" style={{
                borderColor: TEAL_COLOR
                  ...
                  {hotel.email && <a href={`mailto:${hotel.email}`} className="flex items-center gap-2 px-4 py-3 border rounded-lg hover:bg-muted transition-colors" style={{
                borderColor: TEAL_COLOR
              }}>
          ...
          <div className="w-full space-y-4 order-2 lg:order-1">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-muted-foreground mb-2">
                ...
                {distance !== undefined && <span className="text-xs font-medium ml-auto" style={{
                color: TEAL_COLOR
              }}>
                    {distance < 1 ? `${Math.round(distance * 1000)}m away` : `${distance.toFixed(1)}km away`}
                  </span>}
              </div>
              {hotel.place && <p className="text-sm sm:text-xs text-muted-foreground mb-4 sm:mb-2">Place: {hotel.place}</p>}
              
              {/* Mobile Order 3: Action Buttons (Share, Copy Link) */}
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={openInMaps} className="flex-1 h-9" style={{
                borderColor: TEAL_COLOR,
                color: TEAL_COLOR
              }}>
                  <MapPin className="h-4 w-4 md:mr-2" style={{
                  color: TEAL_COLOR
                }} />
                  <span className="hidden md:inline">Map</span>
                </Button>
                <Button variant="outline" size="sm" onClick={handleCopyLink} className="flex-1 h-9" style={{
                borderColor: TEAL_COLOR,
                color: TEAL_COLOR
              }}>
                  <Copy className="h-4 w-4 md:mr-2" style={{
                  color: TEAL_COLOR
                }} />
                  <span className="hidden md:inline">Copy Link</span>
                </Button>
                <Button variant="outline" size="sm" onClick={handleShare} className="flex-1 h-9" style={{
                borderColor: TEAL_COLOR,
                color: TEAL_COLOR
              }}>
                  <Share2 className="h-4 w-4 md:mr-2" style={{
                  color: TEAL_COLOR
                }} />
            ...
            {hotel.description && <div className="bg-card border rounded-lg p-4 sm:p-3">
            ...
            {hotel.amenities && hotel.amenities.length > 0 && <div className="p-4 sm:p-3 border bg-card rounded-lg">
                <h2 className="text-xl sm:text-lg font-semibold mb-4 sm:mb-3">Amenities</h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                  {hotel.amenities.map((amenity, idx) => <div key={idx}
              // --- ADDED rounded-full for 50% border-radius ---
              className="px-3 py-2 text-white rounded-full text-sm flex items-center justify-center text-center min-h-[44px]" style={{
            ...
            {hotel.facilities && hotel.facilities.length > 0 && <div className="p-4 sm:p-3 border bg-card rounded-lg">
                <h2 className="text-xl sm:text-lg font-semibold mb-4 sm:mb-3">Facilities (Room Types)</h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                  {hotel.facilities.map((facility, idx) => <div key={idx}
              // --- ADDED rounded-full for 50% border-radius ---
              className="px-3 py-2 text-white rounded-full text-sm flex flex-col items-center justify-center text-center min-h-[60px] leading-tight" style={{
                backgroundColor: TEAL_COLOR
              }}>
            ...
            {hotel.activities && hotel.activities.length > 0 && <div className="p-4 sm:p-3 border bg-card rounded-lg">
                <h2 className="text-xl sm:text-lg font-semibold mb-4 sm:mb-3">Activities</h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                  {hotel.activities.map((activity, idx) => <div key={idx}
              // --- ADDED rounded-full for 50% border-radius ---
              className="px-3 py-2 text-white rounded-full text-sm flex flex-col items-center justify-center text-center min-h-[60px] leading-tight" style={{
            ...
            {/* Contact Information Section (Always in the right column / bottom of mobile section) */}
            {(hotel.phone_numbers || hotel.email) && <div className="p-4 sm:p-3 border bg-card rounded-lg lg:hidden"> {/* Visible on small screen, hidden on large */}
                <h2 className="text-xl sm:text-lg font-semibold mb-4 sm:mb-3">Contact Information</h2>
                <div className="grid grid-cols-1 sm:grid-cols-1 gap-2">
                  {hotel.phone_numbers?.map((phone, idx) => <a key={idx} href={`tel:${phone}`} className="flex items-center gap-2 px-4 py-3 border rounded-lg hover:bg-muted transition-colors" style={{
                borderColor: TEAL_COLOR
      ...
      <Dialog open={bookingOpen} onOpenChange={setBookingOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <MultiStepBooking onSubmit={handleBookingSubmit} facilities={hotel.facilities || []} activities={hotel.activities || []} isProcessing={isProcessing} isCompleted={isCompleted} itemName={hotel.name} itemId={hotel.id} bookingType="hotel" hostId={hotel.created_by || ""} onPaymentSuccess={() => setIsCompleted(true)} />
        </DialogContent>
      </Dialog>
