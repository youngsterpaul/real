...
import {
  ...
  AlertDialogHeader,
  AlertDialogTitle, 
} from "@/components/ui/alert-dialog"; import { useState, useEffect, useRef, useMemo, useCallback, memo } from "react";
import { useNavigate, Link } from "react-router-dom";
import { SearchBarWithSuggestions } from "@/components/SearchBarWithSuggestions";
import { ListingCard } from "@/components/ListingCard";
import { Card } from "@/components/ui/card";
...
const Index = () => {
  ...
  const [scrollableRows, setScrollableRows] = useState<{
    trips: any[]; hotels: any[]; attractions: any[]; campsites: any[]; events: any[]; accommodations: any[];
  ...
  const tripEventIds = useMemo(() => {
    ...
    listings.forEach(item => { if (item.type === "TRIP" || item.type === "EVENT") ids.push(item.id); });
    return [...new Set(ids)];
  ...
  const [scrollPositions, setScrollPositions] = useState<Record<string, number>>({
    featuredForYou: 0, featuredEvents: 0, featuredCampsites: 0,
    featuredHotels: 0, featuredAttractions: 0, featuredTrips: 0, featuredAccommodations: 0
  });
  ...
  const onTouchStart = useCallback((e: React.TouchEvent) => { setTouchEnd(null); setTouchStart(e.targetTouches[0].clientX); }, []);
  ...
  const onTouchEnd = useCallback((ref: React.RefObject<HTMLDivElement>) => {
    ...
    if (distance > minSwipeDistance) scrollSection(ref, 'right');
    if (distance < -minSwipeDistance) scrollSection(ref, 'left');
  ...
  const fetchScrollableRows = useCallback(async (limit: number) => {
    ...
    try {
      const [tripsData, hotelsData, campsitesData, eventsData, accommodationsData] = await Promise.all([
        supabase.from("trips").select("id,name,location,place,country,image_url,date,is_custom_date,is_flexible_date,available_tickets,activities,type,created_at,price,price_child").eq("approval_status", "approved").eq("is_hidden", false).eq("type", "trip").order("date", { ascending: true }).limit(fetchLimit),
        supabase.from("hotels").select("id,name,location,place,country,image_url,activities,latitude,longitude,created_at,establishment_type").eq("approval_status", "approved").eq("is_hidden", false).limit(fetchLimit),
        supabase.from("adventure_places").select("id,name,location,place,country,image_url,entry_fee,activities,latitude,longitude,created_at").eq("approval_status", "approved").eq("is_hidden", false).limit(fetchLimit),
        supabase.from("trips").select("id,name,location,place,country,image_url,date,is_custom_date,is_flexible_date,available_tickets,activities,type,created_at,price,price_child").eq("approval_status", "approved").eq("is_hidden", false).eq("type", "event").order("date", { ascending: true }).limit(fetchLimit),
        supabase.from("hotels").select("id,name,location,place,country,image_url,activities,latitude,longitude,created_at,establishment_type").eq("approval_status", "approved").eq("is_hidden", false).eq("establishment_type", "accommodation_only").limit(fetchLimit),
      ]);
      setScrollableRows({
        trips: tripsData.data || [], hotels: hotelsData.data || [], attractions: [],
        campsites: campsitesData.data || [], events: eventsData.data || [], accommodations: accommodationsData.data || []
      });
    } catch (error) {
  ...
  const fetchNearbyPlacesAndHotels = useCallback(async () => {
    ...
    const [placesData, hotelsData] = await Promise.all([
      supabase.from("adventure_places").select("id,name,location,place,country,image_url,entry_fee,activities,latitude,longitude,created_at").eq("approval_status", "approved").eq("is_hidden", false).limit(12),
      supabase.from("hotels").select("id,name,location,place,country,image_url,activities,latitude,longitude,created_at").eq("approval_status", "approved").eq("is_hidden", false).limit(12),
    ]);
    const combined = [
    ...
    const withDistance = combined.map(item => {
      ...
      const distance = itemAny.latitude && itemAny.longitude && position
        ? calculateDistance(position.latitude, position.longitude, itemAny.latitude, itemAny.longitude) : undefined;
  ...
  const fetchAllData = useCallback(async (query?: string, offset: number = 0, limit: number = 15) => {
    ...
    const fetchEvents = async () => {
      let dbQuery = supabase.from("trips").select("id,name,location,place,country,image_url,date,is_custom_date,is_flexible_date,available_tickets,activities,type,created_at,price,price_child").eq("approval_status", "approved").eq("is_hidden", false).eq("type", "event").or(`date.gte.${today},is_flexible_date.eq.true`);
      if (query) dbQuery = dbQuery.or(`name.ilike.%${query}%,location.ilike.%${query}%,country.ilike.%${query}%`);
    ...
    const fetchTable = async (table: "hotels" | "adventure_places", type: string) => {
      let dbQuery = supabase.from(table).select(table === "hotels" ? "id,name,location,place,country,image_url,activities,latitude,longitude,created_at" : "id,name,location,place,country,image_url,entry_fee,activities,latitude,longitude,created_at").eq("approval_status", "approved").eq("is_hidden", false);
      if (query) dbQuery = dbQuery.or(`name.ilike.%${query}%,location.ilike.%${query}%,country.ilike.%${query}%`);
    ...
    const fetchTrips = async () => {
      let dbQuery = supabase.from("trips").select("id,name,location,place,country,image_url,date,is_custom_date,is_flexible_date,available_tickets,activities,type,created_at,price,price_child").eq("approval_status", "approved").eq("is_hidden", false).eq("type", "trip");
      if (query) dbQuery = dbQuery.or(`name.ilike.%${query}%,location.ilike.%${query}%,country.ilike.%${query}%`);
      dbQuery = dbQuery.order('date', { ascending: true }).range(offset, offset + limit - 1);
      const { data } = await dbQuery;
  ...
  useEffect(() => {
    ...
    if (cachedData) {
      ...
      setScrollableRows({ trips: cachedScrollable.trips || [], hotels: cachedScrollable.hotels || [], attractions: cachedScrollable.attractions || [], campsites: cachedScrollable.campsites || [], events: cachedScrollable.events || [], accommodations: cachedScrollable.accommodations || [] });
    ...
    const initUserId = async () => { const id = await getUserId(); setUserId(id); };
    initUserId();
  }, [cardLimit, fetchScrollableRows, fetchAllData]);

  useEffect(() => {
    if (!loading && !loadingScrollable && listings.length > 0) setCachedHomePageData({ scrollableRows, listings, nearbyPlacesHotels });
  ...
  const categories = [
    { icon: Tent, title: "Adventures", path: "/category/campsite", bgImage: "/images/category-campsite.webp", description: "Adventure camping spots" },
    { icon: Hotel, title: "Hotels", path: "/category/hotels", bgImage: "/images/category-hotels.webp", description: "Find comfortable stays" },
    { icon: Calendar, title: "Trips", path: "/category/trips", bgImage: "/images/category-trips.webp", description: "Explore guided tours and day trips" },
    { icon: Compass, title: "Events", path: "/category/events", bgImage: "/images/category-events.webp", description: "Discover exciting events" },
    { icon: Home, title: "Stays", path: "/category/accommodation", bgImage: "/images/category-hotels.webp", description: "Rooms & accommodation only" },
  ];
  ...
  const getDisplayItems = useCallback((items: any[], sortedByRating: any[], isTripsOrEvents: boolean = false) => {
    ...
    if (isTripsOrEvents) {
      ...
      result.forEach(item => {
        ...
        const isSoldOut = !item.is_flexible_date && item.available_tickets !== null && item.available_tickets !== undefined && (item.available_tickets <= 0 || bookedCount >= item.available_tickets);
        if (isOutdated || isSoldOut) return;
        if (item.is_flexible_date) flexibleAndAvailable.push(item);
        else fixedDateAvailable.push(item);
      });
  ...
  return (
    // pb-20 = clears mobile bottom nav bar (80px); md:pb-0 = no padding needed on desktop
    <div className="min-h-screen bg-background pb-20 md:pb-0">

      {/* ── Hero Section ── 
          pt-14 pushes content below the fixed header (header = py-2 + h-10 buttons = 56px = Tailwind mt-14).
          We use pt-14 on the wrapper so the hero image still starts at the very top,
          but the TEXT/search inside starts below the header. */}
      {!isSearchFocused && (
        <div
          ref={searchRef}
          className="relative w-full overflow-hidden"
          style={{ height: 'calc(55vh + 56px)', marginTop: 0, paddingTop: '56px' }}
        >
          {/* Hero background */}
          <div className="absolute inset-0 bg-slate-800" />
          <picture>
            <source media="(max-width: 640px)" srcSet="/images/hero-background.webp" type="image/webp" />
            <source media="(max-width: 1024px)" srcSet="/images/hero-background.webp" type="image/webp" />
            <img
          ...
          {/* Search section — positioned below the header space */}
          <div className="absolute inset-0 flex flex-col items-center justify-center pb-32 md:pb-24" style={{ top: '56px' }}>
            <div className="container md:px-4 px-4">
              ...
              <SearchBarWithSuggestions
                ...
                onSuggestionSearch={query => { setSearchQuery(query); fetchAllData(query); setIsSearchFocused(true); }}
          ...
          <div className="absolute bottom-0 left-0 right-0 md:hidden">
            <div className="px-4 py-4 pb-5" style={{ background: 'linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.6) 50%, rgba(0,0,0,0) 100%)' }}>
              <div className="flex flex-row justify-between items-start gap-1 px-2">
                {categories.map((cat, index) => {
                  ...
                  return (
                    <div key={cat.title} onClick={() => navigate(cat.path)} className="flex flex-col items-center cursor-pointer group flex-1 min-w-0">
                      <div className="w-11 h-11 rounded-full backdrop-blur-sm flex items-center justify-center border border-white/30 transition-all group-hover:scale-110" style={{ backgroundColor: `${bgColor}CC` }}>
                        <cat.icon className="h-4 w-4 text-white" />
                      </div>
                      <span className="text-[8px] font-bold uppercase tracking-tight mt-1.5 text-center leading-tight max-w-[60px] truncate" style={{ color: bgColor }}>
                        {cat.title}
                      </span>
                    </div>
                  );
      ...
      {/* Search Bar sticky when focused */}
      {isSearchFocused && (
        <div className="sticky top-14 z-50 bg-background p-4 border-b shadow-md">
          <div className="container md:px-4 px-4 mx-auto">
            <SearchBarWithSuggestions
              value={searchQuery}
              onChange={setSearchQuery}
              onSubmit={() => { if (searchQuery.trim()) fetchAllData(searchQuery); }}
              onSuggestionSearch={query => { setSearchQuery(query); fetchAllData(query); }}
              onFocus={() => setIsSearchFocused(true)}
              onBlur={() => {}}
              onBack={() => { setIsSearchFocused(false); setSearchQuery(""); fetchAllData(); }}
              showBackButton={true}
      ...
      <main className="w-full">
            ...
            <div className="container mx-auto px-4 md:px-6 lg:px-8 py-4 md:py-6">
              <div className="grid grid-cols-5 gap-4 w-full">
                {categories.map((cat, index) => {
                  ...
                  const bgColor = categoryColors[index % categoryColors.length];
                  return (
                    <div key={cat.title} onClick={() => navigate(cat.path)} className="flex flex-col items-center cursor-pointer group border-2 border-slate-200 hover:border-primary rounded-xl p-4 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 bg-background">
                      <div className="h-10 w-10 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform duration-300" style={{ backgroundColor: `${bgColor}CC` }}>
                        <cat.icon className="h-5 w-5 text-white" />
                      </div>
                      <span className="font-bold text-sm text-foreground mt-2 text-center" role="heading" aria-level={3}>{cat.title}</span>
                    </div>
                  );
                })}
              </div>
        ...
        {isSearchFocused && (
          <div className="w-full px-4 md:px-6 lg:px-8 mt-6 pb-20 md:pb-8">
            <h2 className="text-xl md:text-2xl font-bold mb-6">{searchQuery ? 'Search Results' : 'All Listings'}</h2>
            {loading ? (
                ...
                {sortedListings.map((listing, index) => {
                  ...
                  return (
                    <div key={listing.id} className="w-full">
                      <ListingCard id={listing.id} type={listing.type} name={listing.name} location={listing.location} country={listing.country} imageUrl={listing.image_url} price={listing.price || listing.entry_fee || 0} date={listing.date} isCustomDate={listing.is_custom_date} isFlexibleDate={listing.is_flexible_date} isOutdated={isOutdated} isSaved={savedItems.has(listing.id)} onSave={() => handleSave(listing.id, listing.type)} availableTickets={isTripsOrEvents ? listing.available_tickets : undefined} bookedTickets={isTripsOrEvents ? bookingStats[listing.id] || 0 : undefined} showBadge={true} priority={index < 4} hidePrice={listing.type === "HOTEL" || listing.type === "ADVENTURE PLACE"} activities={listing.activities} distance={itemDistance} avgRating={ratingData?.avgRating} reviewCount={ratingData?.reviewCount} />
                    </div>
                  );
                })}
              </div>
        ...
        <div className={`w-full ${isSearchFocused ? 'hidden' : ''}`}>
          {/* Top Destinations / My Location Toggle */}
          <section className="mb-1 md:mb-6">
            <div className="container mx-auto px-4 md:px-6 lg:px-8">
              <div className="mb-1 md:mb-3 mt-1 md:mt-0 items-center justify-between flex flex-row my-[5px] gap-6">
                <span onClick={() => setListingViewMode('top_destinations')} className={`cursor-pointer text-xs md:text-sm font-bold whitespace-nowrap transition-all ${listingViewMode === 'top_destinations' ? 'text-[#DC2626] underline underline-offset-4' : 'text-muted-foreground hover:text-[#DC2626]'}`}>
                  Top Destinations
                </span>
                <div onClick={!locationLoading ? handleMyLocationTap : undefined} className={`flex items-center gap-1.5 cursor-pointer text-xs md:text-sm font-bold whitespace-nowrap transition-all ${listingViewMode === 'my_location' ? 'text-[#DC2626] underline underline-offset-4' : 'text-muted-foreground hover:text-[#DC2626]'} ${locationLoading ? 'opacity-70 cursor-wait' : ''}`}>
                  {locationLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <MapPin className="h-3.5 w-3.5" />}
          ...
          <div className="container mx-auto px-4 md:px-6 lg:px-8 py-6">
            ...
            <section className="mb-2 md:mb-6">
              <div className="mb-1 md:mb-3 flex items-center justify-between bg-gradient-to-r from-primary/10 to-transparent py-2 px-3 rounded-lg border-l-4 border-primary">
                <h2 className="text-[0.9rem] sm:text-2xl font-bold whitespace-nowrap overflow-hidden text-ellipsis min-w-max text-primary">Places to adventure</h2>
                <Link to="/category/campsite" className="text-primary text-xs md:text-sm font-bold hover:underline bg-primary/10 px-2 py-1 rounded-full">View All</Link>
              </div>
              <div className="relative">
                {scrollableRows.campsites.length > 0 && (
                  <>
                    <Button variant="ghost" size="icon" aria-label="Scroll left" onClick={() => scrollSection(featuredCampsitesRef, 'left')} className="absolute left-0 top-1/2 -translate-y-1/2 z-10 h-8 w-8 md:h-10 md:w-10 rounded-full bg-black/50 hover:bg-black/70 text-white"><ChevronLeft className="h-5 w-5 md:h-6 md:w-6" /></Button>
                    <Button variant="ghost" size="icon" aria-label="Scroll right" onClick={() => scrollSection(featuredCampsitesRef, 'right')} className="absolute right-0 top-1/2 -translate-y-1/2 z-10 h-8 w-8 md:h-10 md:w-10 rounded-full bg-black/50 hover:bg-black/70 text-white"><ChevronRight className="h-5 w-5 md:h-6 md:w-6" /></Button>
                  </>
                )}
                <div ref={featuredCampsitesRef} onScroll={handleScroll('featuredCampsites')} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={() => onTouchEnd(featuredCampsitesRef)} className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide pl-0 pr-8 md:pl-2 md:pr-12 scroll-smooth">
                  {loadingScrollable ? (
                    <div className="flex gap-4">{[...Array(5)].map((_, i) => <div key={i} className="flex-shrink-0 w-[75vw] sm:w-[320px] md:w-[380px]"><ListingSkeleton /></div>)}</div>
                  ) : displayCampsites.length === 0 ? (
                    <div className="flex-1 text-center py-8 text-muted-foreground">No adventure places available</div>
            ...
            {/* Hotels */}
            <section className="mb-2 md:mb-6">
              <div className="mb-1 md:mb-3 flex items-center justify-between bg-gradient-to-r from-[#008080]/10 to-transparent py-2 px-3 rounded-lg border-l-4 border-[#008080]">
                <h2 className="text-[0.9rem] sm:text-2xl font-bold whitespace-nowrap overflow-hidden text-ellipsis min-w-max text-[#008080]">Hotels and accommodations</h2>
                <Link to="/category/hotels" className="text-[#008080] text-xs md:text-sm font-bold hover:underline bg-[#008080]/10 px-2 py-1 rounded-full">View All</Link>
              </div>
              <div className="relative">
                {scrollableRows.hotels.length > 0 && (
                  <>
                    <Button variant="ghost" size="icon" aria-label="Scroll left" onClick={() => scrollSection(featuredHotelsRef, 'left')} className="absolute left-0 top-1/2 -translate-y-1/2 z-10 h-8 w-8 md:h-10 md:w-10 rounded-full bg-black/50 hover:bg-black/70 text-white"><ChevronLeft className="h-5 w-5 md:h-6 md:w-6" /></Button>
                    <Button variant="ghost" size="icon" aria-label="Scroll right" onClick={() => scrollSection(featuredHotelsRef, 'right')} className="absolute right-0 top-1/2 -translate-y-1/2 z-10 h-8 w-8 md:h-10 md:w-10 rounded-full bg-black/50 hover:bg-black/70 text-white"><ChevronRight className="h-5 w-5 md:h-6 md:w-6" /></Button>
                  </>
                )}
                <div ref={featuredHotelsRef} onScroll={handleScroll('featuredHotels')} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={() => onTouchEnd(featuredHotelsRef)} className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide pl-0 pr-8 md:pl-2 md:pr-12 scroll-smooth">
                  {loadingScrollable ? (
                    ...
                    <div className="flex-1 text-center py-8 text-muted-foreground">No hotels available</div>
            ...
            {/* Trips */}
            <section className="mb-2 md:mb-6">
              <div className="mb-1 md:mb-3 flex items-center justify-between bg-gradient-to-r from-[#FF0000]/10 to-transparent py-2 px-3 rounded-lg border-l-4 border-[#FF0000]">
                <h2 className="text-[0.9rem] sm:text-2xl font-bold whitespace-nowrap overflow-hidden text-ellipsis min-w-max text-[#FF0000]">Trips and tours</h2>
                <Link to="/category/trips" className="text-[#FF0000] text-xs md:text-sm font-bold hover:underline bg-[#FF0000]/10 px-2 py-1 rounded-full">View All</Link>
              </div>
              <div className="relative">
                {scrollableRows.trips.length > 0 && (
                  <>
                    <Button variant="ghost" size="icon" aria-label="Scroll left" onClick={() => scrollSection(featuredTripsRef, 'left')} className="absolute left-0 top-1/2 -translate-y-1/2 z-10 h-8 w-8 md:h-10 md:w-10 rounded-full bg-black/50 hover:bg-black/70 text-white"><ChevronLeft className="h-5 w-5 md:h-6 md:w-6" /></Button>
                    <Button variant="ghost" size="icon" aria-label="Scroll right" onClick={() => scrollSection(featuredTripsRef, 'right')} className="absolute right-0 top-1/2 -translate-y-1/2 z-10 h-8 w-8 md:h-10 md:w-10 rounded-full bg-black/50 hover:bg-black/70 text-white"><ChevronRight className="h-5 w-5 md:h-6 md:w-6" /></Button>
                  </>
                )}
                <div ref={featuredTripsRef} onScroll={handleScroll('featuredTrips')} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={() => onTouchEnd(featuredTripsRef)} className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide pl-0 pr-8 md:pl-2 md:pr-12 scroll-smooth">
                  {loadingScrollable ? (
                    ...
                    <div className="flex-1 text-center py-8 text-muted-foreground">No trips available</div>
            ...
            {/* Events */}
            <section className="mb-2 md:mb-6">
              <div className="mb-1 md:mb-3 flex items-center justify-between bg-gradient-to-r from-[#FF7F50]/10 to-transparent py-2 px-3 rounded-lg border-l-4 border-[#FF7F50]">
                <h2 className="text-[0.9rem] sm:text-2xl font-bold whitespace-nowrap overflow-hidden text-ellipsis min-w-max text-[#FF7F50]">Sports and events</h2>
                <Link to="/category/events" className="text-[#FF7F50] text-xs md:text-sm font-bold hover:underline bg-[#FF7F50]/10 px-2 py-1 rounded-full">View All</Link>
              </div>
              <div className="relative">
                {scrollableRows.events.length > 0 && (
                  <>
                    <Button variant="ghost" size="icon" aria-label="Scroll left" onClick={() => scrollSection(featuredEventsRef, 'left')} className="absolute left-0 top-1/2 -translate-y-1/2 z-10 h-8 w-8 md:h-10 md:w-10 rounded-full bg-black/50 hover:bg-black/70 text-white"><ChevronLeft className="h-5 w-5 md:h-6 md:w-6" /></Button>
                    <Button variant="ghost" size="icon" aria-label="Scroll right" onClick={() => scrollSection(featuredEventsRef, 'right')} className="absolute right-0 top-1/2 -translate-y-1/2 z-10 h-8 w-8 md:h-10 md:w-10 rounded-full bg-black/50 hover:bg-black/70 text-white"><ChevronRight className="h-5 w-5 md:h-6 md:w-6" /></Button>
                  </>
                )}
                <div ref={featuredEventsRef} onScroll={handleScroll('featuredEvents')} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={() => onTouchEnd(featuredEventsRef)} className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide pl-0 pr-8 md:pl-2 md:pr-12 scroll-smooth">
                  {loadingScrollable ? (
                    ...
                    <div className="flex-1 text-center py-8 text-muted-foreground">No events available</div>
            ...
            {position && sortedNearbyPlaces.length > 0 && (
              <section className="mb-2 md:mb-6">
                <hr className="border-t border-gray-200 my-1 md:my-4" />
                <div className="mb-1 md:mb-3 flex items-center justify-between bg-gradient-to-r from-blue-500/10 to-transparent py-2 px-3 rounded-lg border-l-4 border-blue-500">
                  <div className="flex items-center gap-2">
                    <MapPin className="h-4 w-4 md:h-5 md:w-5 text-blue-500" />
                    <h2 className="text-[0.9rem] sm:text-2xl font-bold whitespace-nowrap overflow-hidden text-ellipsis min-w-max text-blue-500">Nearest to You</h2>
                  </div>
                </div>
                <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide pl-0 pr-8 md:pl-2 md:pr-12 scroll-smooth">
                  {loadingNearby ? (
                    ...
                    const itemAny = item as any;
                    const itemDistance = itemAny.latitude && itemAny.longitude && position ? calculateDistance(position.latitude, position.longitude, itemAny.latitude, itemAny.longitude) : undefined;
                    const ratingData = ratings.get(item.id);
                    return (
                      <div key={item.id} className="flex-shrink-0 w-[75vw] sm:w-[320px] md:w-[380px]">
                        <MemoizedListingCard id={item.id} type={itemAny.type || itemAny.table === 'hotels' ? 'HOTEL' : 'ADVENTURE PLACE'} name={item.name} imageUrl={itemAny.image_url} location={itemAny.location} country={itemAny.country} price={itemAny.entry_fee || 0} date="" onSave={handleSave} isSaved={savedItems.has(item.id)} hidePrice={true} showBadge={true} priority={index === 0} activities={itemAny.activities} distance={itemDistance} avgRating={ratingData?.avgRating} reviewCount={ratingData?.reviewCount} place={itemAny.place} />
                      </div>
                    );
                  })}
                </div>
          ...
          <AlertDialogContent className="max-w-sm">
            <AlertDialogHeader>
              <div className="flex justify-center mb-4">
                <div className="p-3 bg-primary/10 rounded-full"><Navigation className="h-8 w-8 text-primary" /></div>
              </div>
              <AlertDialogTitle className="text-center">Turn On Location</AlertDialogTitle>
              <AlertDialogDescription className="text-center">To see places near you, please enable location access in your device settings.</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter className="flex-col gap-2 sm:flex-col">
              <AlertDialogAction onClick={() => { setShowLocationDialog(false); forceRequestLocation(); }} className="w-full bg-primary hover:bg-primary/90">Try Again</AlertDialogAction>
              <AlertDialogAction onClick={() => { setShowLocationDialog(false); setListingViewMode('top_destinations'); }} className="w-full bg-muted text-muted-foreground hover:bg-muted/80">Continue Without Location</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      </main>
