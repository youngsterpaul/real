...
const TripDetail = () => {
  ...
  const fetchTrip = async () => {
    ...
    try {
      let {
        data,
        error
      } = await supabase.from("trips").select("*").eq("id", id).single();
      if (error && id.length === 8) {
        const {
          data: prefixData,
          error: prefixError
        } = await supabase.from("trips").select("*").ilike("id", `${id}%`).single();
        if (!prefixError) {
          data = prefixData;
          error = null;
        }
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      toast({
        title: "Link Copied!",
        description: user ? "Share this link to earn commission on bookings!" : "Share this trip with others!"
      });
    } catch (error) {
      toast({
        title: "Copy Failed",
  ...
  return <div className="min-h-screen bg-background pb-20 md:pb-0">
      ...
      <div className="relative w-full overflow-hidden md:max-w-6xl md:mx-auto">
        ...
        <Button variant="ghost" onClick={() => navigate(-1)} className="absolute top-4 left-4 z-30 h-10 w-10 p-0 rounded-full text-white md:left-8" style={{
        ...
        if (api) api.on("select", () => setCurrent(api.selectedScrollSnap()));
          ...
          <CarouselContent>
            {displayImages.map((img, idx) => <CarouselItem key={idx}>
                <img src={img} alt={`${trip.name} ${idx + 1}`} loading="lazy" decoding="async" className="w-full h-[60vh] md:h-96 lg:h-[500px] object-cover" />
              </CarouselItem>)}
          </CarouselContent>
        </Carousel>
        ...
        <div className="grid lg:grid-cols-[2fr,1fr] gap-6 sm:gap-4">
          ...
          <div className="space-y-4 sm:space-y-3 lg:mt-0 lg:order-2"> 
            ...
            <div className="space-y-3 p-4 sm:p-3 border bg-card rounded-lg lg:sticky lg:top-20"> 
              ...
              setBookingOpen(true);
            }} disabled={trip.available_tickets <= 0} style={{
            ...
            <div className="flex gap-2">
              ...
              color: TEAL_COLOR
                ...
                <MapPin className="h-4 w-4 mr-2" style={{
                color: TEAL_COLOR
              }} />
                <span>Map</span>
              </Button>
              ...
              color: TEAL_COLOR
                ...
                <Copy className="h-4 w-4 mr-2" style={{
                color: TEAL_COLOR
              }} />
                <span>Copy Link</span>
              </Button>
              ...
              color: TEAL_COLOR
                ...
                <Share2 className="h-4 w-4 mr-2" style={{
                color: TEAL_COLOR
              }} />
                <span>Share</span>
              </Button>
          ...
          <div className="w-full space-y-4 lg:order-1"> 
            ...
            {trip.description && <div className="bg-card border rounded-lg p-4 sm:p-3">
            ...
            {trip.activities && trip.activities.length > 0 && <div className="p-4 sm:p-3 border bg-card rounded-lg">
                <h2 className="text-xl sm:text-lg font-semibold mb-4 sm:mb-3">Included Activities</h2>
                <div className="flex flex-wrap gap-2">
                  {trip.activities.map((activity, idx) => <div key={idx} className="px-3 py-1.5 text-white rounded-full text-xs flex flex-col items-center justify-center text-center" style={{
            ...
            {(trip.phone_number || trip.email) && <div className="p-4 sm:p-3 border bg-card rounded-lg">
                <h2 className="text-xl sm:text-lg font-semibold mb-4 sm:mb-3">Contact Information</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {trip.phone_number && <a href={`tel:${trip.phone_number}`} className="flex items-center gap-2 px-4 py-3 border rounded-lg hover:bg-muted transition-colors" style={{
                borderColor: TEAL_COLOR
              }}>
                  ...
                  color: TEAL_COLOR
                }}>{trip.phone_number}</span>
                    </a>}
                  {trip.email && <a href={`mailto:${trip.email}`} className="flex items-center gap-2 px-4 py-3 border rounded-lg hover:bg-muted transition-colors" style={{
                borderColor: TEAL_COLOR
              }}>
      ...
      <Dialog open={bookingOpen} onOpenChange={setBookingOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <MultiStepBooking onSubmit={handleBookingSubmit} activities={trip.activities || []} priceAdult={trip.price} priceChild={trip.price_child} isProcessing={isProcessing} isCompleted={isCompleted} itemName={trip.name} skipDateSelection={!trip.is_custom_date} fixedDate={trip.date} skipFacilitiesAndActivities={true} itemId={trip.id} bookingType="trip" hostId={trip.created_by || ""} onPaymentSuccess={() => setIsCompleted(true)} />
        </DialogContent>
      </Dialog>
