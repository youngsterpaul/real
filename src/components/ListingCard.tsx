  ...
  return <Card onClick={handleCardClick} className={cn("group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer border rounded-lg bg-card shadow-sm w-full flex flex-col", compact ? "h-[180px] sm:h-[200px]" : "h-[280px] sm:h-[300px] lg:h-[320px]")}>
      ...
      paddingBottom: '65%'
        ...
        quality: 85
                ...
                {onSave && <Button size="icon" onClick={handleSaveClick} className={cn("absolute top-2 right-2 z-20 h-10 w-10 md:h-8 md:w-8 rounded-full p-0 bg-transparent touch-manipulation active:scale-95 transition-transform", "border border-black hover:border-red-500 shadow-sm", "outline-none focus-visible:ring-0 focus-visible:bg-transparent hover:bg-transparent")}>
                        <Heart className={cn("h-5 w-5 md:h-4 md:w-4", isSaved ? "text-red-500 fill-red-500" : "text-white drop-shadow-sm")} />
                    </Button>}
            </div>
